...
# This client is used for Polygon-specific data
...
def get_historical_data(ticker):
    """Fetches historical daily OHLCV data for a given ticker from Polygon."""
...
# --- ✅ V2 ADDITION: Options Chain Snapshot Endpoint ---
...
def get_options_chain(ticker):
    ...
    try:
        ...
        chain = client.list_options_contracts(ticker)
        
        if not chain:
            return jsonify({"message": f"No options chain data found for {ticker}"}), 404
        
        # Format the data to keep only what we need for analysis
        formatted_chain = []
        for contract in chain:
            # Skip contracts with no greeks data
