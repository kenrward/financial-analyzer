...
def analyze_volatility():
    ...
    try:
        ...
        stock_price = float(payload.get('stock_price', 0))
        options_chain = payload.get('options_chain', [])
        hv_30d = float(payload.get('historical_volatility', 0))

        if not options_chain or stock_price == 0 or hv_30d == 0:
             return jsonify({"message": f"Insufficient data provided for volatility analysis of {ticker}."}), 200
        ...
        if valid_dtes.empty:
             return jsonify({"message": f"No options found with at least 25 DTE for {ticker}"}), 200
        ...
        if atm_strike_row.empty:
            return jsonify({"message": f"Could not determine ATM strike for {ticker}"}), 200
            
        atm_iv = atm_strike_row['implied_volatility'].iloc[0] * 100
        ...
        if calls_30d.empty or puts_30d.empty:
            return jsonify({"message": f"Could not find both calls and puts for DTE {nearest_30d_dte}"}), 200
